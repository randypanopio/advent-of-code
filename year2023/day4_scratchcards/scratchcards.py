...
IMPORTANT: double the count after first, pretty straight forward 1 * 2*(n-1)
...
def solution():
    result = []
    for card in get_data(absolute_file_path):
        card = card.split(":")[1] # strip prefix
        ents = card.split("|")
        # or regex lul
        wins = list(filter(None, ents[0].split(" ")))
        my_nums = list(filter(None, ents[1].split(" ")))     
        print(f"wins: \n{wins}")
        print(f"my nums: \n{my_nums}")
        tally = 0
        for n in my_nums:
